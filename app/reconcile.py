...
#     # def process_message(self, topic: str, message: dict):
#     #     trade_id = message.get("trade_id")
#     #     print(f"[ENGINE DEBUG] Received message for trade_id={trade_id} on topic={topic}")

#     #     self.message_buffer[topic][trade_id] = message

#     #     has_exec = trade_id in self.message_buffer['executions']
#     #     has_conf = trade_id in self.message_buffer['confirmations']
#     #     has_pnl = trade_id in self.message_buffer['pnl_snapshot']
#     #     print(f"[ENGINE DEBUG] Buffer status for {trade_id}: exec={has_exec}, conf={has_conf}, pnl={has_pnl}")

#     #     if has_exec and has_conf and has_pnl:
#     #         print(f"[ENGINE DEBUG] All data present. Reconciliation will run for {trade_id}")
#     #         self.reconcile(trade_id)
#     #     else:
#     #         print(f"[ENGINE DEBUG] Missing data for {trade_id}, skipping reconciliation")
...
#             print(f"Warning: Message from topic {topic} missing 'trade_id': {message}")
#             return
#         trade_id = str(trade_id)  # 🔧 ensure consistent key type
...
#                 mismatches.append({
#                     'field': 'pnl_calculation_error',
#                     'reason': f'Error converting PnL related values: {e}'
#                 })
#                 is_matched = False
...
#         # Record latency
#         if 'start_time' in self.trade_store[trade_id]:
#             latency = time.time() - self.trade_store[trade_id]['start_time']
#             if self.reconciliation_latency_histogram:
#                 self.reconciliation_latency_histogram.observe(latency)
...
class ReconciliationEngine:
    ...
    def _perform_reconciliation_and_save(self, trade_id: str, execution: dict, confirmation: dict, pnl: dict = None):
        ...
        # --- Save to DB using the SHARED session ---
        session = self.session # <-- THIS IS THE FIX
        try:
            existing = session.query(ReconciliationResult).filter_by(trade_id=trade_id).first()
            if existing:
                existing.ticker = execution.get('ticker', 'N/A')
